...
class SignUpTrainer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    void signUp() async {
      if (fName.text.isEmpty || lName.text.isEmpty) {
        showSnack('Empty First and Last Name',
            'Please provide your first and last name');
      } else if (dobController.text.isEmpty) {
        showSnack(
            'Invalid Date of Birth', 'Please province a valid Date of Birth');
      } else if (!email.text.isEmail) {
        showSnack('Invalid Email', 'Please province a valid Email');
      } else if ((password.text.length < 8) ||
          (password.text != cPassword.text)) {
        showSnack('Invalid/Mismatching Passwords',
            'Please province a valid and same password');
      } else if (!mobile.text.isPhoneNumber) {
        showSnack(
            'Invalid Mobile Number', 'Please province a valid Phone Number');
      } else if (!tAc.value) {
        showSnack('Not Agreed to Terms and Conditions',
            'Please read and mark as agreed to terms and conditions');
      } else if (countryID == null) {
        showSnack('Invalid Country', 'Please select your country!');
      } else {
        toggle();
        var request = http.MultipartRequest(
            'POST', Uri.parse('https://ion-groups.live/api/users/trainers'));
